...
suppl_table7 <- fread("project/Genome_biology_submission/supplementary_tables/Supplementary_Table7.tsv")


##### Alluvial ######
df_classification <- dplyr::select(tukiainen[tukiainen$Gene_name %in% suppl_table7$gene,], Gene_name, Reported_XCI_status)
...
df_gylemo <- dplyr::select(suppl_table7, gene, category)
...
ggsave2(filename = paste0(plot_dir, "Figure_2k.pdf"), width=3, height = 4,
        plot_grid(ggplot(smash_fin, aes(alluvium = gene, x = axes, stratum = category)) + 
                    ...
                    labs(x="",y="gene count")+
                    geom_text(stat="count",aes(col= category, label=..count.., group = interaction(category, axes)), show.legend = F)+
                    ...
                    scale_color_manual(values = c("PAR" = "yellow", "Escape" = "red", "Inactive" = "#869a9a", "Variable" = "purple", "Unknown" = "blue", "Undeterminable" = "green", "curated variable" = "pink", "not_investigated" = "black"))
        )
)
