...
# ASE tables dir
file_dir_ASE_screen <- dir("supplementary_data/screen/ase", full.names = T) 
...
df_ase_screen$name <- gsub(df_ase_screen$name, pattern = "supplementary_data/screen/ase/", replacement = "")

# add participant column
df_ase_screen$participant <- sub(df_ase_screen$name, pattern = "^([^-]*-[^-]*).*", replacement = "\\1")

# WES data dir
file_dir_WES_screen <- dir("supplementary_data/screen/wes", full.names = T) 
...
# remove dir from file name column
df_wes_screen$name <- gsub(df_wes_screen$name, pattern = "supplementary_data/screen/wes/", replacement = "")
