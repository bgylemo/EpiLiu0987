...
# our data
# ASE tables dir
WES_file_dir_ASE_screen <- dir("project/data/cXCI_women/redo_SNP_calling/ASEReadCounter_output/WES/Q20", full.names = T, recursive = F)

WGS_file_dir_ASE_screen <- dir("project/data/cXCI_women/redo_SNP_calling/ASEReadCounter_output/WGS/Q20", full.names = T) 
...
WES_df_ase_screen$name <- gsub(WES_df_ase_screen$name, pattern = "project/data/cXCI_women/redo_SNP_calling/ASEReadCounter_output/WES/Q20/high_qual_", replacement = "")
WGS_df_ase_screen$name <- gsub(WGS_df_ase_screen$name, pattern = "project/data/cXCI_women/redo_SNP_calling/ASEReadCounter_output/WGS/Q20/high_qual_", replacement = "")
...
# WES data dir
WES_file_dir_WES_screen <- dir("project/Genome_biology_submission/supplementary_data/ASEReadCounter_output/WES_readcounts", full.names = T)
WGS_file_dir_WES_screen <- dir("project/Genome_biology_submission/supplementary_data/ASEReadCounter_output/WGS_readcounts", full.names = T) 
...
# remove dir from file name column
WES_df_wes_screen$name <- gsub(WES_df_wes_screen$name, pattern = "project/Genome_biology_submission/supplementary_data/ASEReadCounter_output/WES_readcounts/", replacement = "")
WGS_df_wes_screen$name <- gsub(WGS_df_wes_screen$name, pattern = "project/Genome_biology_submission/supplementary_data/ASEReadCounter_output/WGS_readcounts/", replacement = "")
...
df_TPM_tissues <- melt(fread("project/Genome_biology_submission/supplementary_data/22_11_07_expression_matrix_unfiltered.tsv"))
df_TPM_tissues$variable <- gsub(df_TPM_tissues$variable, pattern = "\\.", replacement = "-")
meta_full <- fread("project/Genome_biology_submission/annotation_data/metadata_RNAseq_full_for_tissues.tsv")
...
suppl_table6 <- unique(dplyr::select(df_TPM_femmes, -age ,   -sex ,-has_genotype ,-has_rnaseq))

suppl_table6_chrX <- suppl_table6[suppl_table6$gene %in% gtf_raw$gene_name,]

write.table(suppl_table6_chrX, file = paste0(table_dir, "Supplementary_Table6.tsv"), quote = F, row.names = F, sep = "\t")



########## Supplementary table 7 ##############
suppl_table7 <- dplyr::select(dcast_suppl_table5_counts, gene, category)

suppl_table7[suppl_table7$gene %in% c("ANOS1", "PRKX", "ARSD", "MED14", "NAA10", "CA5B", "SMC1A", "DIP2KB", "TMEM187", "TRAPPC2", "UBA1"),]$category <- "curated variable"
suppl_table7[suppl_table7$category == "variable",]$category <- "Undeterminable"
...
suppl_table8 <- unique(dplyr::select(df_merged_meta_unfiltered, name, tissue_id))

names(suppl_table8)[names(suppl_table8) == 'name'] <- 'sample_id'

# Write table.
write.table(suppl_table8, file = paste0(table_dir, "Supplementary_Table8.tsv"), quote = F, row.names = F, sep = "\t")